<script setup>
	...
	//Variables
	const output = ref('')
	const valid = computed(() => props.validation ? props.validation(output.value) : true)
	...
	const controlInputFloat = computed(() => {
		return [
			props.floatLabel || props.floatLeft || props.floatRight ? 'eit-input__display' : '', 
    ...
    <template v-if="props.floatLabel">
      <label class="eit-label-float">
		...
		<template v-if="props.floatLeft">
			<span class="eit-input__display__icon eit-input__display__float-left--icon">
				<slot name="float-left"></slot>
			</span>
		</template>
		<template v-if="props.floatRight">
			<span class="eit-input__display__icon eit-input__display__float-right--icon">
