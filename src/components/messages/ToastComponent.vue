...
<template>
	<div 
		class="eit-toast__container"
		:class="configToast"
	>
		<div 
			class="eit-toast box-shadow"
			:class="`eit-toast--${props.data.type}`"
		>
			<div class="d-flex">
...
<style lang="scss">
    ...
    &__text {
      ...
      @extend .eit-font__size--x1;
		...
		&--success {
			& {
				border-color: lch(from var(--eit-color-green) l c h / .2);
				background-color: lch(from var(--eit-color-green) l c h / .2);
			}
      .eit-toast__title, .eit-toast__icon {
      ...
      .eit-toast__title, .eit-toast__icon {
		...
		&--warning {
			& {
				border-color: lch(from var(--eit-color-yellow) l c h / .2);
